    ...
    for(size_t i = 0; i < bullets.size(); i++){
        ...
        if(fabs(xdist) < dodgeDistance.x && fabs(ydist) < dodgeDistance.y){
            if(ydist <= 0){  // Bullet above, move down
                ...
                // Bullet above but 290 < angle < 340. Sensiable to dodge up.
                if( -(M_PI*290)/180 > bullets[i].angle && bullets[i].angle -(M_PI*340)/180)
                    return cpVect{0.0, DODGEINTENSITY};
                return cpVect{0.0, -DODGEINTENSITY};
            }else if (ydist > 0){            // Bullet bellow, move up

                // Bullet above but 20 < angle < 70. Sensiable to dodge down.
                if( -(M_PI*20)/180 > bullets[i].angle && bullets[i].angle < -(M_PI*70)/180)
                    return cpVect{0.0, -DODGEINTENSITY};
                return cpVect{0.0, DODGEINTENSITY};
            }else{
